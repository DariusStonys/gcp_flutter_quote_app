...
class RandomQuotesState extends State<RandomQuotes> {
  ...
  _getRandomQuote() async {
    var url = 'https://us-central1-dewrn-app.cloudfunctions.net/getrandomquote-function';
    ...
    try {
      ...
      if (response.statusCode == HttpStatus.ok) {
        var json = await response.transform(pack.utf8.decoder).join();
        var data = pack.json.decode(json);
        result = data['quote'] + "\n-- " + data['person'];
      } else {
        result =
